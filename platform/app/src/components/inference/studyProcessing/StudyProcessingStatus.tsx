  ...
  if (initialSnapshotStatus === 'error') {
    return (
      <div
        ...
        role="status"
        aria-label="Processing status unavailable"
        title={initialSnapshotError || 'The processing status service is unavailable.'}
  ...
  if (initialSnapshotStatus === 'ready') {
    return (
      <div
        className="min-w-[208px]"
        role="status"
        aria-label="No processing status yet"
        title="This study has not entered the processing pipeline."
