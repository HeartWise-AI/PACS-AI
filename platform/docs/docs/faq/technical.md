...
The study panel can expose the `StudyBrowserSort` dropdown when study-browser settings are enabled. The dropdown lets you sort the series by a specific value and comes with default sorting functions such as Series Number, Series Image Count, and Series Date.
...
The study panel exposes sorting controls through `StudyBrowserSort` when study-browser settings are enabled. This will look like:
