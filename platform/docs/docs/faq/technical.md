...
The `StudyBrowserSort` control is available in the study panel and allows sorting series by a specific value. It comes with 3 default sorting functions: Series Number, Series Image Count, and Series Date.
...
You can customize sorting behavior in the study browser using `customizationService`, as shown below:
