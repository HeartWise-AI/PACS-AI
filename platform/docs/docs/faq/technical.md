...
The study panel includes a sort dropdown in the settings row (when panel settings are enabled for the mode). It comes with 3 default sorting functions: Series Number, Series Image Count, and Series Date.
...
Use the study browser sort dropdown in the panel settings row. This will look like:
