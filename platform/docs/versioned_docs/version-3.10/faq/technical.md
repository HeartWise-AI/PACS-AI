...
The StudyBrowser sort dropdown is available when the study browser settings row is enabled.
The component comes with default sorting functions (Series Number, Series Image Count, and
Series Date), and you can add your own custom sort functions.
...
You can customize sorting in the study panel by providing values for `studyBrowser.sortFunctions`.
This will look like:
