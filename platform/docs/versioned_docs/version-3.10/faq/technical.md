...
The study panel includes the `StudyBrowserSort` dropdown for sorting series by a specific value. It comes with 3 default sorting functions: Series Number, Series Image Count, and Series Date.
...
The study panel includes a sort dropdown that looks like this:
