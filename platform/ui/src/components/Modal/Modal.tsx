...
const Modal = ({
  ...
  const renderHeader = () =>
    title && (
      <header className="drag-handle flex items-center rounded-tl rounded-tr bg-[#151815] px-[20px] py-[13px]">
        <Typography
          variant="h6"
          color="primaryLight"
        ...
        className={
          contentDimensions
            ...
            : // NOTE: This is a PACS changes
              'ohif-scrollbar modal-content overflow-y-auto rounded-bl rounded-br bg-[#151815] px-[20px] pt-2 pb-[20px]'
        }
      >
        {children}
      </section>
  ...
  return (
    <ReactModal
      className={
        containerDimensions
          ? `relative !bg-[#151815] text-white outline-none ${containerDimensions}`
          : 'relative max-h-full w-11/12 !bg-[#151815] text-white outline-none lg:w-10/12 xl:w-9/12'
      }
      overlayClassName={
        movable
